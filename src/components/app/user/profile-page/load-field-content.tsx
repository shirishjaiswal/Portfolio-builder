...
import { ReactSelectOption } from "@/components/app/admin-control/page-navigation/static-data";
import {
  reactSelectStyles,
  styleForTags,
} from "@/components/app/admin-control/page-navigation/styles";
...
const InputFields = ["text", "link", "email", "phone"];
...
const LoadFieldContent: React.FC<LoadFieldContentProps> = ({
  ...
  const handleEndDate = (setDate: DateValue | null) =>
    setEndDate(
      userProfileKey,
      userProfileSectionsKey,
      userProfileSectionIndex,
  ...
  const handleInputTexts = (
    e?: React.ChangeEvent<HTMLInputElement>,
    selectionType?: string
  ) => {
    let value = e?.target?.value ?? null;

    if (selectionType === "email" && value) {
      value = value.toLowerCase().split(" ").join("");
    }

    if (selectionType === "phone" && value) {
      try {
      ...
      {InputFields.includes(sectionField.selectionType) && (
        <Input
          className={`text-roboto text-sm z-0 ${
            sectionField.selectionType === "link" ? "w-2/6" : "w-1/5"
          }`}
          isClearable
          ...
          onChange={(e) => handleInputTexts(e, sectionField.selectionType)}
      ...
      {sectionField.selectionType === "duration" && (
        <div className="flex w-7/12 gap-8">
          ...
          <DatePicker
            ...
            isDisabled={sectionField.onGoing ?? false}
          />
          <Checkbox
            key={"ongoing"}
            ...
            radius="sm"
            isDisabled={sectionField.endDate ? true : false}
          >
            {"OnGoing"}
          </Checkbox>
        </div>
